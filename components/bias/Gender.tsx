...
export default function GenderBias() {
  const vizref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (vizref.current?.children.length == 0) {
      const viz = new TableauViz();
      viz.src =
        "https://public.tableau.com/views/ProjectVizPart2/GenderBias?:language=en-GB&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link";
